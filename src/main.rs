...
fn main() {
    let mut gemini_api_keys = Option::<String>::None;
    ...
    loop {
        if gemini_api_keys.is_none() {
            match terminal_api_key_input() {
                Ok(api_key) => {
                    gemini_api_keys = Some(api_key);
        ...
        let all_subtitles_file = subtitles_queue.len();
...
fn terminal_api_key_input() -> Result<String, String> {
        ...
        match io::stdin().read_line(&mut max_length_buffer) {
            Ok(_) => {
                ...
                match max_length.parse::<usize>() {
                    ...
                    Err(_) => {
                        LOGGER.bold_error("Invalid input.\nPlease enter a valid input: ");
                        io::stdout().flush().unwrap();
                        continue;
                    }
                }
