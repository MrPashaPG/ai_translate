use std::{ffi::OsStr, fs, path::PathBuf, sync::LazyLock, env}; // Added env for robust env var handling
...
pub fn collect_subtitles_path(dir_path: &str, subtitles_queue: &mut FifoQueue<PathBuf>) {
    ...
    let mut entries: Vec<_> = match entries_result {
        Ok(read_dir) => read_dir
            .filter_map(|res| {
                if res.is_err() {
                    LOGGER.warning(format!("Error reading an entry in '{}', skipping entry.", dir_path).as_str());
                }
        ...
        Err(e) => {
            LOGGER.error(
                format!("Error reading directory contents of '{}': {}. Skipping this directory.", dir_path, e).as_str(),
            );
            return; 
        }
    };
    ...
    let target_dir_name = env::var(target_dir_env_name)
        .unwrap_or_else(|_| {
            LOGGER.warning(format!("Environment variable '{}' not set. Using default 'Translated_Subtitles'.", target_dir_env_name).as_str());
            "Translated_Subtitles".to_string()
        });
    let original_sub_dir_name = env::var(original_sub_dir_env_name)
        .unwrap_or_else(|_| {
            LOGGER.warning(format!("Environment variable '{}' not set. Using default 'Original_Subtitles_Backup'.", original_sub_dir_env_name).as_str());
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            {
                LOGGER.info(format!("➡️  Skipping scan of system folder: '{}'", path.display()).as_str());
                continue;
            }
            ...
            if let Some(path_str) = path.to_str() {
                ...
                LOGGER.warning(format!("Path '{}' contains non-UTF8 characters, skipping.", path.display()).as_str());
            }

        } else if path.is_file() && path.extension().map_or(false, |ext| ext.eq_ignore_ascii_case("srt")) {
            let file_name = path
                .file_name()
                .unwrap_or_default()
                .to_str()
            ...
            if !file_name.contains("_fa.") && !file_name.ends_with("_fa.srt") { // More specific check for "_fa"
                LOGGER.debug(format!("Subtitle file found: {}", path.display()).as_str());
                subtitles_queue.enqueue(path);
            } else {
                LOGGER.info(format!("File '{}' appears to be already translated (contains '_fa'). Skipping.", file_name).as_str());
            }
        }
    }
...
pub fn subtitle_exists_in_target_dir(original_path: &PathBuf) -> bool {
    ...
    if !fa_dir.is_dir() {
        LOGGER.debug(format!("Target directory '{}' does not exist yet (for checking file existence).", fa_dir.display()).as_str());
        return false;
    }
    ...
    if exists {
        LOGGER.debug(format!("File '{}' found in target directory.", fa_file_path.display()).as_str());
    }
    exists
}